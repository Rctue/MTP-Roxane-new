    ...
    def startRobotTask(self):
        ...
        while task_state == "not_done":
            ...
            for can_nr in self.list_nr_cans:
                ...
                # print("---------", robot_reach_time, self.robot_end_time)
            ...
            for can in self.env.list_cans:
                self.env.box.inBox(can)
...
class Environment:
        ...
        while True:
            ...
            for e in events:
                ...
                if task_state == "not_done":
                    self.humanTaskEvents(e) # One full task works, but it errors upon the start of the second task because there are no cans on the table (in debug)
    ...
    def humanTaskEvents(self, e):
        ...
        self.human_back_to_start = False

        for self.predicted_can_nr, can in enumerate(self.list_cans):
            # Check if any object already in hand. If so, break out of for loop
            # if any(human_state == "in_hand" for human_state in self.human_state):
            #     break
            # else:
                ...
                if not self.human_reached:
                    if not any(human_state == "in_hand" for human_state in self.human_state):
                        ...
                        if abs(self.predicted_position[0] - can.getPos()[0]) < 0.05:
                            if self.human_state[self.predicted_can_nr] != "human_boxed":
        ...
        if self.checkPointStart(trajectory['start'], trajectory['current']):
            self.human_back_to_start = True
            print("Human back at start") # Always seems to be at start

        if self.human_back_to_start:
            self.human_reached = False
    ...
    def robotGrabEvents(self, can, can_nr):
        ...
        if self.panda_type == 0:
            ...
            #print(f"-----Object state {can_nr} at start: ", self.state[can_nr])
            ...
            if self.checkAvailability(can_nr):
                ...
                # Put can into box (ONLY IF AVAILABLE)
    ...
    def anticipateHuman(self, can):
        if self.human_state[can_nr] == "predicted_target":
            ...
            for can_nr, can in self.list_cans:
                print(f"-----Object state {can_nr} (replan): ", self.state[can_nr])
    ...
    def putCanInBox(self, can, can_nr):
        ...
        if self.box.inBox(can):
            ...
            # self.boxed_cans.append(can)
            # print("Robot boxed cans: ", self.boxed_cans)

        # Return to base
        self.panda.moveToBase()
        # print(f"-----Object state {can_nr} at end: ", self.state[can_nr])
        # print(f"-----Human state {can_nr} at end: ", self.state[can_nr])
        print("Object State after robot finish: ", self.state)
